...
class GeometryValue < GeometryExpression
  ...
  def preprocess_prog
    self # no pre-processing to do here by default
  end

  def eval_prog env 
    self # all values evaluate to self by default
  end
...
class LineSegment < GeometryValue
  ...
  def preprocess_prog
    if real_close_point(@x1, @y1, @x2, @y2) then
      Point.new(@x1, @y1)
    elsif !real_close(@x1, @x2) && @x1 > @x2 then
      LineSegment.new(@x2, @y2, @x1, @y1)
    elsif real_close(@x1, @x2) && @y1 > @y2 then
      LineSegment.new(@x2, @y2, @x1, @y1)
    else
      self
    end
...
class Let < GeometryExpression
  ...
  def eval_prog env
    new_env = env.clone
    if !((new_env.assoc @s).nil?) then
      new_env.delete(new_env.assoc(@s))
    end 
