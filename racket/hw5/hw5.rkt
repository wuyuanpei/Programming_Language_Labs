         ...
         (let ([clo (eval-under-env (call-funexp e) env)]
           ...
           (if (closure? clo)
               (let* ([func (closure-fun clo)]
                     [env-arg (cons (cons (fun-formal func) arg) (closure-env clo))])
                 (eval-under-env (fun-body func)
                                 (if (fun-nameopt func)
                                     (cons (cons (fun-nameopt func) clo)
                                            env-arg)
                                     env-arg)))
               (error "MUPL function called is not a closure")))]
        [(apair? e)
    ...
    (fun #f "func"
         (fun "f" "list"
              (ifaunit (var "list") (aunit)
                       (apair (call (var "func") (fst (var "list"))) (call (var "f") (snd (var "list"))))))))
...
(define mupl-mapAddN 
  (mlet "map" mupl-map
        (fun #f "i"
             (fun #f "list"
                  (call (call (var "map") (fun #f "num" (add (var "num") (var "i")))) (var "list"))))))
